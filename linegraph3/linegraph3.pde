...
void setup() {
  ...
  data = new FloatTable("sumeVsNon.tsv");
...
void drawTitle() {
  ...
  textSize(20);
  textAlign(LEFT);
  String title = "Summary Ejectments vs. Other Small Claims";
...
void drawLineLabels() {
  String[] lineName = {"Ejectments", "Other"};
  
  // Get y offsets for the first point in each line
  float[] stops = new float[lineName.length];
  ...
  for(int j = 0; j < lineName.length; j++){
    text(lineName[j], plotX2 + 10, stops[j]+2);
  }
